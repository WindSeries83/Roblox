---
name: retrospective
description: "Sprint or milestone retrospective from completed work, velocity, and blockers."
...
- Travail par lots : retrospective typiquement en fin de lot. (« Sprint » ci-dessous = « lot » : plans datés dans `docs/superpowers/plans/`, exécution suivie dans `.superpowers/sdd/`.)
...
- For sprint retrospectives: `docs/production/retrospectives/retro-[sprint-slug]-*.md`
  (also check `docs/production/sprints/sprint-[N]-retrospective.md` as an alternate location)
- For milestone retrospectives: `docs/production/retrospectives/retro-[milestone-name]-*.md`

If a matching file is found, use the platform's question UI:
...
- Sprint plans: `docs/production/sprints/` (pour ce projet : plans de lot dans `docs/superpowers/plans/`)
- Milestone definitions: `docs/production/milestones/`

**Also check for `docs/production/sprint-status.yaml`**: if it exists, read it alongside the sprint plan. It is the authoritative source for actual story completion status (status: done, completed dates, blockers). Use it as the primary source for completion metrics in Phase 3. Fall back to markdown scanning only if the yaml does not exist. Note discrepancies between the yaml and the sprint plan (e.g., stories in yaml not in plan, or vice versa).

**If the file does not exist or is empty**, output:

> "No sprint data found for [sprint/milestone]. Check `.superpowers/sdd/` for lot tracking
> first, or provide the sprint details manually."

Then use the platform's question UI to present two options:
...
Read previous retrospectives (if any) from `docs/production/retrospectives/` to check:
...
Ask: "May I write this to `docs/production/retrospectives/retro-sprint-[N]-[date].md`?" (or `docs/production/retrospectives/retro-[milestone-name]-[date].md` for milestone retrospectives)

If yes, write the file, creating the `docs/production/retrospectives/` directory if needed. Verdict: **COMPLETE** — retrospective saved.
...
Use the platform's question UI:
- Prompt: "Retrospective complete. The action items and velocity data are ready. Would you like to start next-lot planning now with this data pre-loaded?"
- Options:
  - `[A] Yes — open next-lot planning with retro action items and velocity delta pre-populated`
  - `[B] No — I'll reference the retrospective file manually when I'm ready`

If the user selects [A]: Proceed to plan the next lot via `writing-plans` (superpowers), passing the retrospective file path and a summary of the action items and velocity change so the planner can reference them.

- If this was a milestone retrospective, formally assess readiness for the next phase before starting it.
