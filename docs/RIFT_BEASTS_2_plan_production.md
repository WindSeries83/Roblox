...
- [x] **À trancher (chat Roblox)** : décidé — chat moderne TextChatService bas-gauche (`ChatWindowConfiguration.VerticalAlignment = Bottom` dans `Ui.luau`). Tant que `ChatVersion = LegacyChatService`, l'ancien chat est désactivé/détruit (il bloquait les clics de l'UI). **Reste à faire (utilisateur)** : flipper `TextChatService → ChatVersion = TextChatService` dans l'Explorer (10 s), puis `Ctrl+S`.
...
- **Restes** : flip `ChatVersion = TextChatService` (utilisateur, 10 s) + `Ctrl+S` du place (utilisateur) + validation visuelle finale (agent `eyes` indisponible en fin de session).
