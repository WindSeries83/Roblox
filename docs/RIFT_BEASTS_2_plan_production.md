...
- [x] **Audit over-engineering appliqué (23/08, 170/170 tests)** : suppression de Types.luau (jamais requis), Log.Entries (écrit/jamais lu), SaveProfile/GetEssence/EssenceService.ComputeRate (zéro appelant), branches tutoriel mortes (« Click », onglets), faux cap saisonnier 1e18, double victoire cristal faille (le tick proximité suffit), gardes nil mortes ×7, grantErr jamais assigné. Déduplication : CheckCooldown → `Gameplay.CheckCooldown`, find-créature ×6 → `Gameplay.FindCreature`, payload listings ×2 → `ListingsPayload()`, Top-N classements ×2 → `TopPayload()`, quêtes ×4 → `ProgressAndUnlock`, récompenses saison ×2 → `Season.RewardFor`, piste season panel ×2 → local `Track()`, sous-onglets Duels/Quêtes → `UiKit.SubTabs`, Hex/TIER_NAMES/taux de drop → helpers UiKit, RarityRank client ×2 → Gameplay, EconomySim réutilisant les modules partagés, RenderSelectors/Boucle FloatText (TweenService)/LayoutTabs par signal. **Conservé volontairement** : hooks nommés SeasonService:OnX (sémantiques), doublon COLORS dans HoverCard (ce module est requis par un test serveur ; UiKit tire Audio client-only — ne pas créer ce lien). Suite : 170/170 (−3 tests des fonctions supprimées), boot + UI vérifiés en playtest.
