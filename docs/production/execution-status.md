...
|---|---|---|---|
| 0 - vertical slice | En cours | Headless 267/267, 8 skips live ; CI 33966168926 verte sur ef2daff ; smoke Studio frais passe sur ef2daff (72/72 remotes, boucle oeuf, Rift, UI) | timings 15-20/30-40 min ; focus/B/Echap manuel ; persistance publiee |
...
- STUDIO : smoke frais execute sur `ef2daff` apres synchronisation Rojo. Boot serveur/client sans erreur applicative ; `RemoteRegistry` = 72/72 `RemoteEvent`, aucun manquant, extra ou mauvaise classe. `Net.Sync.Policy` recu par le client contient `Loaded=true`, `ArePaidRandomItemsRestricted=false`, `IsPaidItemTradingAllowed=true` (mock Studio permissif, pas un compte restreint reel). Achat/obtention d'un `CommonEgg`, selection/placement, incubation, hatch et reception de `HatchResult` verifies (`Hurle-lune`, `Rare`, mutation `None`) ; taux de rarete affiches (`Commun 75 %`, `Peu commun 20 %`, `Rare 5 %`) et taux de mutation affiches (`None 88 %`, `Shadow 10 %`, `Frost 1.7 %`, `Golden 0.2 %`, `Prismatic 0.07 %`, `Corrupted 0.02 %`, `Celestial 0.01 %`). Le stub `SafeStore` Studio a ete verifie en memoire (`SetAsync`/`GetAsync`, valeur 123). Rift ouvert par helper dev temporaire, entree `TwilightGrove`, `RiftSync`/`ExpeditionSync` et attaque verifies (`GuardianHP` 180 -> 126). Les panneaux Gameplay, Oeufs et Boutique sont presents et leurs textes de taux/etat se rendent correctement, avec TopBar/ActionBar. ServerBoost active temporairement apres un vrai hatch : taux Essence `14.5 -> 18.125` avec bonus `0.25`, `ProbabilityContext` inchange ; les achats reels restent desactives (`SKU 0`). Les seuls messages attendus sont l'indisponibilite Roblox API/MemoryStore en place Studio non publiee.
...
- Studio Edit courant : le serveur Rojo unique écoute sur `localhost:34872`, la source `SafeStore` est synchronisée vers `ef2daff`, et les helpers temporaires ont été supprimés (`RiftInterval=nil`, aucun `SmokePolicyCapture`). `RemoteRegistry` Edit contient 62 remotes sources sans extra ni doublon ; le serveur en crée les 72 attendus au boot.
- Preuve Studio courante : le Play frais de `ef2daff` valide le runtime local ; il ne couvre pas la policy d'un vrai compte restreint, les achats/receipts publiés ni le multi-serveur.
